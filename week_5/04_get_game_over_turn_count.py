...
# 말이 쌓일 수 있습니다. -> 맵에 말이 쌓이는 걸 저장
# 쌓인 순서대로 이동 -> stack 써야함.
# 현재 맵에 어떻게 말이 쌓일 지 저장하기 위해서는
# chese_map

def get_game_over_turn_count(horse_count, game_map, horse_location_and_directions):
    n = len(chess_map)
    current_stacked_hourse_map = [
        [
            [] for _ in range(n)
        ] for _ in range(n)
    ]
    for i in range(horse_count):
        ...
        current_stacked_hourse_map[r][c].append(i)
    turn_count = 1
    ...
    while turn_count <= 1000:
        for horse_index in range(horse_count):
            ...
            for i in range(len(current_stacked_hourse_map[r][c])):
                current_stacked_horse_index = current_stacked_hourse_map[r][c][i]
                if horse_index == current_stacked_horse_index:
                    moving_horse_index_array = current_stacked_hourse_map[r][c][i:]
                    current_stacked_hourse_map[r][c] = current_stacked_hourse_map[r][c][:i]
                    break
            ...
            for moving_horse_index in moving_horse_index_array:
                current_stacked_hourse_map[new_r][new_c].append(moving_horse_index)
                horse_location_and_directions[moving_horse_index][0], horse_location_and_directions[moving_horse_index][1] = new_r, new_c
    #턴이 진행되는 중 말이 4개 이상 쌓이는 순간 게임이 종료
            if len(current_stacked_hourse_map[new_r][new_c]) >= 4:
                return turn_count
        turn_count += 1
